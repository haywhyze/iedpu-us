...
export default function EditEventModal({
  ...
  useEffect(() => {
    if (exco) {
      const {
        name, position, bio, id, image,
      } = exco;
      setValues({ name, position, bio });
      setImage(image);
    }
  }, [exco]);
  ...
  const editExecutive = () => {
    ...
    db.collection('executives')
      ...
      .then(() => {
        ...
        toast.success('Advisory Council Member successfully updated');
      })
      .catch((error) => {
        toast.error(`Error updating Advisory Council member, ${error.message}`);
      });
  };
  return (
    <div>
          ...
          >
            ...
            >
              <CardBody>
                <GridContainer>
                  <GridItem xs={12}>
                    <h2>Update Advisory Council Member</h2>
                  </GridItem>
                  <GridItem xs={12}>
                    <CustomInput
                      labelText="Name"
            ...
            <DialogActions className={classes.modalFooter}>
              <Button onClick={editExecutive} color="primary">
